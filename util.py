...
def light_source_directions_new(direc_path):
    with open(direc_path, 'r') as file:
        lines = file.readlines()

    light_directions = [list(map(float, line.split())) for line in lines]

    L = np.array(light_directions, dtype=float)
    return L